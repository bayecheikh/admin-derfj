  ...
  <v-form class="" v-model="valid" ref="form" enctype="multipart/form-data">
    <v-stepper v-model="step" vertical>
      ...
      <v-stepper-content step="3">
        <v-card class="mx-auto mb-5 pl-10 pt-10 pr-10 pb-5" flat>
          ...
          <v-simple-table class="custom-ligne-bloc-2">
            <thead>
              <tr>
                <th class="text-left">Intitulé de l'activité</th>
                <th class="text-left">Description de l'activité</th>
                <th class="text-left">Nombre de bénéficiaire Homme</th>
                <th class="text-left">Nombre de bénéficiaire Femme</th>
                <th class="text-left">Type de matériel utilisé</th>
                <th class="text-left">-</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, i) in LigneActivites" :key="item.id">
        ...
        <v-btn color="primary" @click="step = 4"> Suivant </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>
      ...
      <v-stepper-content step="4">
        <v-card class="mx-auto mb-5 pl-10 pt-10 pr-10 pb-5" flat>
          ...
          <v-simple-table class="custom-ligne-bloc-2">
            <thead>
              <tr>
                <th class="text-left">Difficulté rencontrée</th>
                <th class="text-left">Solution trouvée</th>
                <th class="text-left">Suivi nécessaire</th>
                <th class="text-left">-</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, i) in LigneContraintes" :key="item.id">
                <td>{{ item.difficulte_rencontres }}</td>
                <td>{{ item.solution_trouves }}</td>
                <td>{{ item.suivie_necessaires }}</td>
                <td>
                  <v-icon @click="deleteFindContrainte(i)">mdi-close</v-icon>
                </td>
              </tr>
            </tbody>
          </v-simple-table>
        </v-card>
        <v-btn color="primary" @click="step = 5"> Suivant </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>
      ...
      <v-stepper-content step="5">
        <v-card class="mx-auto mb-5 pl-10 pt-10 pr-10 pb-5" flat>
          <v-row v-for="(item, i) in fichiers" :key="item.id">
            <v-col md="6" lg="6" sm="12">
              <v-text-field
        ...
        <v-btn color="primary" @click="step = 6"> Suivant </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>
      ...
      <v-stepper-content step="6">
        ...
        <v-btn color="primary" @click="step = 7"> Suivant </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>

      <!-- Step:7 Données specifique -->
      <v-stepper-step :complete="step > 7" step="7" editable>
        Données spécifiques
      </v-stepper-step>
      <v-stepper-content step="7">
        <v-card class="container pl-10 pt-10 pb-10 pr-10 mb-5 border-grey" flat>
          <v-row>
            ...
            <v-col lg="2" md="2" sm="2">
              <v-radio-group v-model="model.unite_superficie" row>
                <v-radio label="m2" value="m2"></v-radio>
                <v-radio label="ha" value="ha"></v-radio>
              </v-radio-group>
            </v-col>
            <v-col lg="12" md="12" sm="12">
              <v-textarea
            ...
            <v-col md="6" lg="6" sm="12">
              <v-expansion-panels class="mb-2 mt-0">
                <v-expansion-panel>
                  <v-expansion-panel-header>
                    <h3>Valorisation</h3>
                  </v-expansion-panel-header>
        ...
        <v-btn color="primary" @click="step = 8"> Suivant </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>

      <!-- Step:8 Resumé -->
      <v-stepper-step :complete="step > 8" step="8" editable>
        Résumé
      </v-stepper-step>
      <v-stepper-content step="8">
            ...
            <v-col md="12" sm="12" lg="12" text-md-left>
              <div class="row">
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Région : </span>{{ resume.nom_region }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Departement : </span>{{ resume.nom_departement }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Commune : </span>{{ resume.nom_commune }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    ...
                    >{{
                      modelBeneficiaire.prenom_beneficiaire +
                      " " +
                      modelBeneficiaire.nom_beneficiaire
                    }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Référence Projet : </span
                    >{{ modelProjet.reference_projet }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Secteur : </span>{{ resume.nom_secteur }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Latitude : </span>{{ model.latitude }}
                  </p>
                </div>
                <div class="col-md-4 my-0 py-0">
                  <p class="info-profil">
                    <span>Longitude : </span>{{ model.longitude }}
                  </p>
                </div>
                <div class="col-md-12 col-sm-12 col-lg-12 my-5 py-0">
                  <p class="info-profil">
                    <span>Activités de la semaine</span>
                  </p>
                  <v-simple-table class="custom-ligne-bloc-2">
                    <thead>
                      <tr>
        ...
        <v-btn color="primary" @click="controleValidation"> Soumettre </v-btn>
        <v-btn text> Annuler </v-btn>
      </v-stepper-content>
    </v-stepper>
  </v-form>
...
export default {
  ...
  data: () => ({
    ...
    listsecteurs: [
      ...
      {
        id: 4,
        nom_secteur: "Situation des recouvrements au niveau départemental",
      },
      {
        id: 5,
        nom_secteur: "Organisation des sessions d'animation économique",
      },
  ...
  methods: {
    ...
    submitForm() {
      ...
      console.log("Données formulaire +++++", {
        ...this.model,
        libelle_secteur: this.libelle_secteur,
      ...
      let formData = new FormData();
          ...
          formData.append("activites", JSON.stringify(this.LigneActivites));
          formData.append("contraintes", JSON.stringify(this.LigneContraintes));

      validation &&
        this.$msasApi
          .post("/enquettes",  formData)
          .then((res) => {
    ...
    submitLigneFichier() {
      ...
      this.fichiers.push({
        id: this.countrow_fichier,
        libelle_fichier: this.libelle_fichiers[this.countrow_fichier],
        input_fichier: this.inputfichiers[this.countrow_fichier],
      });
    ...
    getBenefByTerm(param) {
      this.$msasApi
        .get("/beneficiaire-by-term/" + param)
        .then(async (response) => {
    ...
    async changeBeneficiaire(value) {
      ...
      //resumé
      this.resume.nom_beneficiaire =
        value.prenom_beneficiaire + " " + value.nom_beneficiaire;
    },
