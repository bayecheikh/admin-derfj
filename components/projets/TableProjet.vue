<template>
  <div>
    ...
    <v-data-table
     v-model="selected"
      :headers="headers"
      :items="tab=='tout'?listprojets : listprojets.filter(projet => projet.status === tab)"
  ...
  export default {
    ...
    methods: {
      getList(page){
        ...
        .then(async (response) => {
            console.log('Données reçu projets ++++++: ', response.data.data)
       ...
       getResult(page,param){
         this.progress=true
         this.$msasApi.get('/projet-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
      ...
      actveDesactiveprojet(id) {
        console.log('------------- projet active',id)
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$get('/active_projet/'+id)
        .then(async (response) => {   
