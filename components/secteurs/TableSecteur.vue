...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listsecteurs : listsecteurs.filter(secteur => secteur.status === tab)"
  item-key="id"
  items-per-page="5"
  class="flat pt-4"
  :loading="listsecteurs.length?false:true" 
...
>
  ...
  <template v-slot:[`item.sous_secteurs`]="{ item }">
      <v-chip
        color="primary"
        small
        outlined
        class="my-1 mr-1"
        v-for="sous_secteur in item.sous_secteurs"  :key="sous_secteur.value"
      >
        {{ sous_secteur.libelle }}
      </v-chip>
  </template>
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      ...
      headers: 'secteurs/headersecteurs'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('secteurs/getDetail',item)
        this.$router.push('/secteurs/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('secteurs/getDetail',item)
        this.$router.push('/secteurs/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/secteurs/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('secteurs/deletesecteur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let secteur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('secteurs/initdetail',secteur)
          this.$router.push('/secteurs/detail/'+secteur.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let secteur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('secteurs/initdetail',secteur)
          this.$router.push('/secteurs/modifier/'+secteur.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
