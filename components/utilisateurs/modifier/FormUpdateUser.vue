    ...
    export default {
      ...
      methods: {
        ...
        .then(async (response) => {
            ...
            this.model.secteur_id = response.data.secteur[0]?.id
        ...
        let checkRole = this.model.roles.filter(item => (item && item.name === 'expert-sectoriel' || item && item.name === 'admin_structure' || item && item.name === 'DGES' || item && item.name === 'directeur_eps')).length;
        if(checkRole==1)
