...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listfinancement_activites : listfinancement_activites.filter(financement_activite => financement_activite.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listfinancement_activites.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      ...
      headers: 'financement_activites/headerfinancement_activites'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('financement_activites/getDetail',item)
        this.$router.push('/financement_activites/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('financement_activites/getDetail',item)
        this.$router.push('/financement_activites/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        ...
        .then(async (response) => { 
            this.$store.dispatch('financement_activites/deletefinancement_activite',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          this.$store.commit('financement_activites/initdetail',financement_activite)
          this.$router.push('/financement_activites/detail/'+financement_activite.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let financement_activite = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_activites/initdetail',financement_activite)
          this.$router.push('/financement_activites/modifier/'+financement_activite.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
