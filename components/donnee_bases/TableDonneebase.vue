...
import RechercheDonneebase from '@/components/donnee_bases/RechercheDonneebase';
  export default {
    components: {
      RechercheDonneebase
    },
    ...
    computed: mapGetters({
      listdonnee_bases: 'donnee_bases/listdonnee_bases',
      headers: 'donnee_bases/headerdonnee_bases',
      totalpage: 'donnee_bases/totalpage',
      perpage: 'donnee_bases/perpage',
      datasearch: 'donnee_bases/datasearch',
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      getList(page){
          this.progress=true
          this.$msasApi.$get('/donnee_bases?page='+page)
        .then(async (response) => {
            ...
            this.$store.dispatch('donnee_bases/getTotalPage',totalPages)
            this.$store.dispatch('donnee_bases/getPerPage',response.data.per_page)
            this.$store.dispatch('donnee_bases/getList',response.data.data)
       ...
       getResult(page,param){
         this.progress=true
         this.$msasApi.get('/donnee_base-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            console.log('Données reçus++++++++++++',response.data.data.data)
            await this.$store.dispatch('donnee_bases/getList',response.data.data.data)
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('donnee_bases/getTotalPage',totalPages)
            this.$store.dispatch('donnee_bases/getPerPage',response.data.data.per_page)
      ...
      actveDesactiveUser(id) {
        ...
        this.$msasApi.$get('/active_donnee_base/'+id)
        .then(async (response) => {   
      ...
      visualiserItem (item) {
        this.$store.dispatch('donnee_bases/getDetail',item)
        this.$router.push('/donnee_bases/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('donnee_bases/getDetail',item)
        this.$router.push('/donnee_bases/modifier/'+item.id);
      },
      async deleteItem () {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$delete('/donnee_bases/'+this.activeItem.id)
        .then(async (response) => {             
            this.$store.dispatch('donnee_bases/deleteUtilisateur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          this.$store.commit('donnee_bases/initdetail',donnee_base)
          this.$router.push('/donnee_bases/detail/'+donnee_base.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let donnee_base = this.selected.map(function(value){ return value})[0]
          this.$store.commit('donnee_bases/initdetail',donnee_base)
          this.$router.push('/donnee_bases/modifier/'+donnee_base.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
