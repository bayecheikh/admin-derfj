...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listbailleurs : listbailleurs.filter(bailleur => bailleur.status === tab)"
  item-key="id"
  items-per-page="5"
  class="flat pt-4"
  :loading="listbailleurs.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      ...
      headers: 'bailleurs/headerbailleurs'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('bailleurs/getDetail',item)
        this.$router.push('/bailleurs/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('bailleurs/getDetail',item)
        this.$router.push('/bailleurs/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/bailleurs/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('bailleurs/deletebailleur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let bailleur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('bailleurs/initdetail',bailleur)
          this.$router.push('/bailleurs/detail/'+bailleur.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let bailleur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('bailleurs/initdetail',bailleur)
          this.$router.push('/bailleurs/modifier/'+bailleur.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
