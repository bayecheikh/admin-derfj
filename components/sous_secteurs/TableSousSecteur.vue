...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listsous_secteurs : listsous_secteurs.filter(sous_secteur => sous_secteur.status === tab)"
  item-key="id"
  items-per-page="5"
  class="flat pt-4"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('sous_secteurs/getDetail',item)
        this.$router.push('/sous_secteurs/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('sous_secteurs/getDetail',item)
        this.$router.push('/sous_secteurs/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/sous_secteurs/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('sous_secteurs/deletesous_secteur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let sous_secteur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('sous_secteurs/initdetail',sous_secteur)
          this.$router.push('/sous_secteurs/detail/'+sous_secteur.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let sous_secteur = this.selected.map(function(value){ return value})[0]
          this.$store.commit('sous_secteurs/initdetail',sous_secteur)
          this.$router.push('/sous_secteurs/modifier/'+sous_secteur.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
