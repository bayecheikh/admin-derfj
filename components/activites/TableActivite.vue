...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listactivites : listactivites.filter(activite => activite.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listactivites.length?false:true" 
    ...
    methods: {
      ...
      editItem (item) {   
       ...
       deleteItem (item) {
        ...
        this.$msasApi.$delete('/activites/'+this.activeItem.id)
        .then(async (response) => { 
