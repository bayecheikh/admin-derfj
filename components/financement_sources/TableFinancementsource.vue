...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listfinancement_sources : listfinancement_sources.filter(financement_source => financement_source.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listfinancement_sources.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      ...
      headers: 'financement_sources/headerfinancement_sources'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('financement_sources/getDetail',item)
        this.$router.push('/financement_sources/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('financement_sources/getDetail',item)
        this.$router.push('/financement_sources/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/financement_sources/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('financement_sources/deletefinancement_source',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          this.$store.commit('financement_sources/initdetail',financement_source)
          this.$router.push('/financement_sources/detail/'+financement_source.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let financement_source = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_sources/initdetail',financement_source)
          this.$router.push('/financement_sources/modifier/'+financement_source.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
