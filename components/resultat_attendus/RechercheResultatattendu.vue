  ...
  export default {
    ...
    methods: {
      ...
      submitForm () {
        ...
        this.$store.commit('listresultat_attendus/initdatasearch',this.model.dataSearch)
      ...
      getList(){
        this.page=1
        this.progress=true
        this.$store.dispatch('listresultat_attendus/getDataSearch',null)
         this.$msasApi.$get('/listresultat_attendus?page=1')
        .then(async (response) => {
          this.page=1
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('listresultat_attendus/getTotalPage',totalPages)
            this.$store.dispatch('listresultat_attendus/getPerPage',response.data.data.per_page)
            this.$store.dispatch('listresultat_attendus/getList',response.data.data)
        }).catch((error) => {
      ...
      getResult(page,param){
        this.page=1
         this.progress=true
         this.$msasApi.get('/listresultat_attendus-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            console.log('Données reçus++++++++++++',response.data.data.data)
            await this.$store.dispatch('listresultat_attendus/getList',response.data.data.data)
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('listresultat_attendus/getTotalPage',totalPages)
            this.$store.dispatch('listresultat_attendus/getPerPage',response.data.data.per_page)
