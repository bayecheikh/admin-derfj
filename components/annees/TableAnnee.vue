...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listannees : listannees.filter(annee => annee.status === tab)"
  :single-select="singleSelect"
  item-key="id"
  items-per-page="5"
  class="flat pt-4"
  :loading="listannees.length?false:true" 
...
>
  ...
  <template v-slot:[`item.type_annees`]="{ item }">
      <v-chip
        color="primary"
        small
        outlined
        class="my-1 mr-1"
        v-for="type_annee in item.type_annees"  :key="type_annee.value"
      >
        {{ type_annee.libelle }}
      </v-chip>
  </template>
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('annees/getDetail',item)
        this.$router.push('/annees/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('annees/getDetail',item)
        this.$router.push('/annees/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/annees/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('annees/deleteannee',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let annee = this.selected.map(function(value){ return value})[0]
          this.$store.commit('annees/initdetail',annee)
          this.$router.push('/annees/detail/'+annee.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let annee = this.selected.map(function(value){ return value})[0]
          this.$store.commit('annees/initdetail',annee)
          this.$router.push('/annees/modifier/'+annee.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
