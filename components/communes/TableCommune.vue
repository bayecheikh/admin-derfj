...
<v-data-table
  v-model="selected"
  :headers="headers"
  :items="tab=='tout'?listcommunes : listcommunes.filter(commune => commune.status === tab)"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      ...
      headers: 'communes/headercommunes'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      getList(){
        this.progress = true
      this.$msasApi.$get('/communes')
      .then(async (response) => { 
        console.log('Données communes reçu+++++++++++',response)
            this.$store.dispatch('communes/getList',response.data)
          }).catch((error) => {
      ...
      visualiserItem (item) {   
        this.$store.dispatch('communes/getDetail',item)
        this.$router.push('/communes/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('communes/getDetail',item)
        this.$router.push('/communes/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/communes/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('communes/deleteDepartement',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let commune = this.selected.map(function(value){ return value})[0]
          this.$store.commit('communes/initdetail',commune)
          this.$router.push('/communes/detail/'+commune.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let commune = this.selected.map(function(value){ return value})[0]
          this.$store.commit('communes/initdetail',commune)
          this.$router.push('/communes/modifier/'+commune.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
