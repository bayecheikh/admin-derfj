  ...
  <div>
    ...
    <v-tabs-items v-model="tab">
      <v-tab-item v-for="item in tabItems"  :key="item.value" >
          <table-donneebase :tab="item.value"></table-donneebase> 
      </v-tab-item>          
    </v-tabs-items>
  </div>
</template>
<script>
import TableDonneebase from '@/components/cible_fin_projets/TableDonneebase'
  export default {
    components: {
      TableDonneebase
    },
    ...
    methods:{
      goToAddDonnebase() {      
        this.$router.push('/cible_fin_projets/addDonneebase');
      },
    }
  }
</script>
