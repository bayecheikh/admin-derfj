<template>
  <div>
    <v-card-title class="col-12">
      <recherche-donneebase></recherche-donneebase>
      <!-- <v-text-field
        v-model="search"
    ...
    <v-data-table
     v-model="selected"
      :headers="headers"
      :items="tab=='tout'?listdonnee_bases : listdonnee_bases.filter(donnee_base => donnee_base.status === tab)"
...
import RechercheDonneebase from '@/components/cible_fin_projets/RechercheDonneebase';
  export default {
    components: {
      RechercheDonneebase
    },
    ...
    computed: mapGetters({
      listdonnee_bases: 'cible_fin_projets/listdonnee_bases',
      headers: 'cible_fin_projets/headerdonnee_bases',
      totalpage: 'cible_fin_projets/totalpage',
      perpage: 'cible_fin_projets/perpage',
      datasearch: 'cible_fin_projets/datasearch',
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      getList(page){
          this.progress=true
          this.$msasApi.$get('/cible_fins?page='+page)
        .then(async (response) => {
            console.log('Données reçu donnee_bases ++++++: ', response.data.data)
            let totalPages = Math.ceil(response.data.total / response.data.per_page)
            this.$store.dispatch('cible_fin_projets/getTotalPage',totalPages)
            this.$store.dispatch('cible_fin_projets/getPerPage',response.data.per_page)
            this.$store.dispatch('cible_fin_projets/getList',response.data.data)
       ...
       getResult(page,param){
         this.progress=true
         this.$msasApi.get('/cible_fin-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            console.log('Données reçus++++++++++++',response.data.data.data)
            await this.$store.dispatch('cible_fin_projets/getList',response.data.data.data)
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('cible_fin_projets/getTotalPage',totalPages)
            this.$store.dispatch('cible_fin_projets/getPerPage',response.data.data.per_page)
      ...
      actveDesactiveUser(id) {
        console.log('------------- donnee_base active',id)
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$get('/active_cible_fin/'+id)
        .then(async (response) => {   
      ...
      visualiserItem (item) {
        this.$store.dispatch('cible_fin_projets/getDetail',item)
        this.$router.push('/cible_fin_projets/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('cible_fin_projets/getDetail',item)
        this.$router.push('/cible_fin_projets/modifier/'+item.id);
      },
      async deleteItem () {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$delete('/cible_fin_projets/'+this.activeItem.id)
        .then(async (response) => {             
            this.$store.dispatch('cible_fin_projets/deleteUtilisateur',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let donnee_base = this.selected.map(function(value){ return value})[0]
          this.$store.commit('cible_fin_projets/initdetail',donnee_base)
          this.$router.push('/cible_fin_projets/detail/'+donnee_base.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let donnee_base = this.selected.map(function(value){ return value})[0]
          this.$store.commit('cible_fin_projets/initdetail',donnee_base)
          this.$router.push('/cible_fin_projets/modifier/'+donnee_base.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
