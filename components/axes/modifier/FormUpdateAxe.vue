...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    mounted: function() {
      this.model.id = this.detailaxe.id
      this.model.nom_axe = this.detailaxe.nom_axe
    },
    computed: mapGetters({
      detailaxe:'axes/detailaxe'
    }),
    data: () => ({
      loading: false,
      message:null,
    ...
    methods: {
      submitForm () {
        ...
        validation && this.$msasApi.put('/axes/'+this.model.id, {...this.model})
          .then((res) => {    
            this.$store.dispatch('toast/getMessage',{type:'success',text:res.data.message || 'Ajout réussi'})
            this.$router.push('/axes');
          })
          .catch((error) => {
