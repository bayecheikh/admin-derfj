<template>
  <div>
    <v-card-title class="col-12">
      <recherche-beneficiaire></recherche-beneficiaire>
      <!-- <v-text-field
        v-model="search"
    ...
    <v-data-table
     v-model="selected"
      :headers="headers"
      :items="tab=='tout'?listbeneficiaires : listbeneficiaires.filter(beneficiaire => beneficiaire.status === tab)"
...
import RechercheBeneficiaire from '@/components/beneficiaires/RechercheBeneficiaire';
  export default {
    components: {
    ...
    computed: mapGetters({
      listbeneficiaires: 'beneficiaires/listbeneficiaires',
      headers: 'beneficiaires/headerbeneficiaires',
      totalpage: 'beneficiaires/totalpage',
      perpage: 'beneficiaires/perpage',
      datasearch: 'beneficiaires/datasearch',
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      getList(page){
          this.progress=true
          this.$msasApi.$get('/beneficiaires?page='+page)
        .then(async (response) => {
            console.log('Données reçu beneficiaires ++++++: ', response.data.data)
            let totalPages = Math.ceil(response.data.total / response.data.per_page)
            this.$store.dispatch('beneficiaires/getTotalPage',totalPages)
            this.$store.dispatch('beneficiaires/getPerPage',response.data.per_page)
            this.$store.dispatch('beneficiaires/getList',response.data.data)
       ...
       getResult(page,param){
         this.progress=true
         this.$msasApi.get('/beneficiaire-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            console.log('Données reçus++++++++++++',response.data.data.data)
            await this.$store.dispatch('beneficiaires/getList',response.data.data.data)
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('beneficiaires/getTotalPage',totalPages)
            this.$store.dispatch('beneficiaires/getPerPage',response.data.data.per_page)
      ...
      actveDesactivebeneficiaire(id) {
        console.log('------------- beneficiaire active',id)
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$get('/active_beneficiaire/'+id)
        .then(async (response) => {   
      ...
      visualiserItem (item) {
        this.$store.dispatch('beneficiaires/getDetail',item)
        this.$router.push('/beneficiaires/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('beneficiaires/getDetail',item)
        this.$router.push('/beneficiaires/modifier/'+item.id);
      },
      async deleteItem () {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        this.$msasApi.$delete('/beneficiaires/'+this.activeItem.id)
        .then(async (response) => {             
            this.$store.dispatch('beneficiaires/deleteBeneficiaire',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let beneficiaire = this.selected.map(function(value){ return value})[0]
          this.$store.commit('beneficiaires/initdetail',beneficiaire)
          this.$router.push('/beneficiaires/detail/'+beneficiaire.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let beneficiaire = this.selected.map(function(value){ return value})[0]
          this.$store.commit('beneficiaires/initdetail',beneficiaire)
          this.$router.push('/beneficiaires/modifier/'+beneficiaire.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
