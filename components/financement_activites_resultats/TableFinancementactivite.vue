...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listfinancement_activites_resultats : listfinancement_activites_resultats.filter(financement_activite => financement_activite.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listfinancement_activites_resultats.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      listfinancement_activites_resultats: 'financement_activites_resultats/listfinancement_activites',
      headers: 'financement_activites_resultats/headerfinancement_activites'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('financement_activites_resultats/getDetail',item)
        this.$router.push('/financement_activites_resultats/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('financement_activites_resultats/getDetail',item)
        this.$router.push('/financement_activites_resultats/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/financement_activites/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('financement_activites_resultats/deletefinancement_activite',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let financement_activite = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_activites_resultats/initdetail',financement_activite)
          this.$router.push('/financement_activites_resultats/detail/'+financement_activite.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let financement_activite = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_activites_resultats/initdetail',financement_activite)
          this.$router.push('/financement_activites_resultats/modifier/'+financement_activite.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
