...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listfinancement_sources_resultats : listfinancement_sources_resultats.filter(financement_source => financement_source.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listfinancement_sources_resultats.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      listfinancement_sources_resultats: 'financement_sources_resultats/listfinancement_sources',
      headers: 'financement_sources_resultats/headerfinancement_sources'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('financement_sources_resultats/getDetail',item)
        this.$router.push('/financement_sources_resultats/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('financement_sources_resultats/getDetail',item)
        this.$router.push('/financement_sources_resultats/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/financement_sources_resultats/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('financement_sources_resultats/deletefinancement_source',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let financement_source = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_sources_resultats/initdetail',financement_source)
          this.$router.push('/financement_sources_resultats/detail/'+financement_source.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let financement_source = this.selected.map(function(value){ return value})[0]
          this.$store.commit('financement_sources_resultats/initdetail',financement_source)
          this.$router.push('/financement_sources_resultats/modifier/'+financement_source.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
